...
-- // Module // --
local Module = { NUMBER_SUFFIXES = NUMBER_SUFFIXES }
...
function Module:FormatForTimer(seconds)
	...
	return (minutes > 9 and minutes or "0" .. minutes) .. ":" .. (seconds > 9 and seconds or "0" .. seconds)

	-- (hours>9 and hours or '0'..hours)..':'..(minutes>9 and minutes or '0'..minutes)..':'..(seconds>9 and seconds or '0'..seconds)
end
...
function Module:FromRomanNumeral(Numeral)
	...
	if Numeral == "N" then
		return 0
	elseif
		CheckNumOfCharacterInString(Numeral, "V") >= 2
		or CheckNumOfCharacterInString(Numeral, "L") >= 2
		or CheckNumOfCharacterInString(Numeral, "D") >= 2
	then --(#{string.find(Numeral, "V*.V")} >= 2) or (#{string.find(Numeral, "L*.L")} >= 2) or (#{string.find(Numeral, "D*.D")} >= 2) then
		return nil
	end
	local Last = "Z"
	local Count = 1
	...
	while Pointer <= #Numeral do
		...
		if Pointer <= #Numeral - 1 then
			...
			if NextDigit > Digit then
				if
					not SpecialRomanDigit[Numeral]
					or NextDigit > (Digit * 10)
					or CheckNumOfCharacterInString(Numeral, Numeral) > 3
				then
					return nil
				end
				MaxDigit = Digit - 1
...
-- Returns the % value of the decimal (0 -> 1)
...
-- Lerp start value towards end value, alpha decimal of the way.
function Module:Lerp(start, goal, alpha)
	return ((goal - start) * alpha) + start
end

-- Map a value from a range into another range, e.g. say I have a value that can range from 0 to 100, and I want to make that scale into 69 to 420, this can do it.
function Module:MapTo(x, inMin, inMax, outMin, outMax)
	return (x - inMin) * (outMax - outMin) / (inMax - inMin) + outMin
end

-- Mapping a given value to a new value that is between 0 and 1
function Module:Map01(x, min, max)
	return (x - min) / max
end
